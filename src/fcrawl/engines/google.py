...
class GoogleEngine(SearchEngine):
    ...
    async def setup_context_async(self, context, locale: Optional[str] = None) -> None:
        """Async version: Set Google-specific cookies for locale preference"""
        lang_code = "en"
        region_code = "US"
        if locale:
            parts = locale.split("-")
    ...
    async def handle_consent_async(self, page) -> None:
        """Async version: Handle Google cookie consent popup"""
        try:
            for selector in [
                "button:has-text('Accept all')",
                "button:has-text('Accept')",
    ...
    async def extract_results_async(self, page) -> list[SearchResult]:
        """Async version: Extract search results from Google SERP"""
        results = []

        # Google's result structure: div[data-snf='x5WNvb'] contains title/URL
        result_containers = await page.locator("div[data-snf='x5WNvb']").all()

        # Fallback to div.yuRUbf if data-snf not found
        if not result_containers:
            result_containers = await page.locator("div.yuRUbf").all()
